...
public class ConnectFourCanvas extends Canvas implements MouseListener, Runnable{
    ...
    // cells with the winning pieces
    boolean[][] winningPieces = new boolean[row][col];
    
    // columns that have blocks in them
    Vector<Integer> flaggedCol = new Vector<Integer>();
    
    // Pieces above blocker 
    boolean[][] aboveBlocker = new boolean[row][col];     
    
    // animation after blocker disappears
  	boolean fallingAnimation;	
  	
  	// if the falling piece after blocker disappears is Red
  	boolean fallingRed;       
  	
  	// where the piece falls after blocker disappears 	
  	int dropAt;               
    
  	// size of an 'imaginary' box that contains a piece
    int size = 80;            
    int border = 20;
    
    // Row and Column where the falling piece is landing
    int currentR, currentC;   
    
    // acceptable piece while checking if the user wins
    String acceptable;        
    ...
    // check if blocker is in use
    boolean blockerPiece;
    
    // check which blocker in use
    boolean blockerY = false; 
    boolean blockerR = false;
    
    // Player turn: true = red , false = yellow
    boolean turn = true;      
    
    // if winner is decided
    boolean winnerCanvas = false;  
    ...
    public boolean diagonalTestTopBottom(int r, int c){            
    	...
    	if (diagonalTestTopBottomUp(r,c) + diagonalTestTopBottomDown(r,c) - 1 >= 4){
    		return true;
    	}
    	else{
    		winningPieces = new boolean[row][col];
    ...
    public boolean diagonalTestBottomTop(int r, int c){
        // Diagonal test of four piece sequence
    	if (diagonalTestBottomTopUp(r,c) + diagonalTestBottomTopDown(r,c) - 1 >= 4){
    		return true;
    	}
    	else{
    		winningPieces = new boolean[row][col];
    ...
    public boolean horizTest(int row, int col){    	
    	// combines left and right test to check 4 piece sequence
 		if((horizTestRight(row,col) + horizTestLeft(row,col)) - 1 >=4){
 			return true;
 		}
 		else{
    		winningPieces = new boolean[row][col];
